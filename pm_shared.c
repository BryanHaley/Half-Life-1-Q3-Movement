...
void PM_Friction (void)
{
	...
	VectorCopy(vel, vec);
	if ( !(pmove->flags & FL_DUCKING & FL_WATERJUMP & FL_FROZEN & FL_ONTRAIN) )
	{
	...
	if (pmove->waterlevel <= 1) 
	{
		/*if (pml.walking && !(pml.groundTrace.surfaceFlags & SURF_SLICK)) 
		{
			...
			{*/
				...
				drop += control * pm_friction*pmove->frametime;
			/*}
		}*/
	}
	...
	if (pmove->waterlevel) 
	{
		drop += speed * pm_waterfriction*pmove->waterlevel*pmove->frametime;
	}
